...
const UserProfileDetail = ({ profileData, handlePatchUser, handleDeleteUser }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      game_master: profileData.game_master || false,
      current_password: '',
      new_password: '',
    },
    ...
    onSubmit: (values) => {
      ...
      handlePatchUser(profileData.id, payload)
        .then((res) => {
          if (!res.ok) {
  ...
  const handleDelete = () => {
    ...
    if (confirmDelete) {
      handleDeleteUser(profileData.id);
      onUnauthorized();
      navigate('/login');
    }
  };
  ...
  return (
    <div className="user-profile" key={profileData.id}>
      {!isEditMode ? (
        <div className="profile-info">
         <h1>
          ...
          <p>
            ...
            <span className="value">{profileData.game_master ? 'Yes' : 'No'}</span>
          </p>
